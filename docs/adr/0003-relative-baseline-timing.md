...
- 알려진 한계: canary 크기가 작을수록 배수가 실제보다 작게 나오는 문제가 실측으로 확인됐다. 지금 "기본 체크" 크기(batch=1, seq=8)는 device 확인용 PASS/FAIL 판정에는 문제없지만, WARN(배수 기반) 판정의 신뢰도는 낮을 수 있다 — `architecture.md` §7의 probe 기반 정밀화로 향후 개선 예정이다.
