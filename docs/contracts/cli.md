...
재확인한 항목은 `results`에서 **재확인 결과로 교체된다**. 이때 CLI가 얹은 값(`model_name`·`batch_size`·`seq_len`)과 실행한 `fix`는 유지되고 `reverified: true`가 붙는다. 교체 전 판정은 `previous_verdict`로 함께 실린다 — 교체하고 나면 화면에 "수정 후"만 남아 `--yes`가 무엇을 바꿨는지 알 수 없기 때문이다. 재확인하지 않은 나머지 항목은 1차 판정 그대로 남는다.
...
| `fix` | FIX 대상으로 뽑힌 항목에만 | `suggest_fix()` 반환값. `--yes`로 재확인한 뒤에는 **재확인 결과로 다시 계산해 교체한다** — 1차 fix는 이미 실행해 성공했을 수 있어 그대로 두면 끝난 일을 다시 안내하게 된다(#148). 재확인이 PASS면 이 키가 사라진다. 실행한 명령은 `notices`의 `자동 수정 실행: <command>`에 남는다 |
| `reverified` | `--yes`로 재확인한 항목에만 | `True`. 표제에 `(재확인)`을 붙이는 데 쓰인다 |
| `previous_verdict` | `--yes`로 재확인한 항목에만 | 재확인 **전**의 `verdict`. 재확인 블록에 `수정 전 ✖ FAIL → 수정 후 ✔ PASS` 한 줄을 그리는 데 쓰인다. 없으면 그 줄을 생략한다 |
