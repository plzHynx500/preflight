...
>
> **실측 (2026-08-23, 두 번째 조합)** — Issue #3이 요구한 "GPU/CPU 조합 여럿" 중 두 번째 데이터. 실제 프로덕션 코드(`worker._measure`)를 저장소 밖 디스포저블 venv에 그대로 설치해 CPU에서 float32 대 bfloat16을 비교했다(`hidden=4096, batch=1, seq=8`, 3회 반복 median).
>
> | 항목 | 값 |
> |---|---|
> | GPU | NVIDIA GeForce RTX 5070 Laptop (8GB) · driver 595.79 · CUDA 13.2 |
> | CPU | AMD Ryzen AI 7 350 (Zen5) — `avx512f/bw/vnni` 등은 있으나 **`avx512_bf16` 전용 명령어는 없음**(py-cpuinfo로 확인) |
> | RAM | 32GB |
> | 스택 | torch 2.11.0+cu128(GPU 활성 경로) / 2.13.0+cpu(CPU 단독 경로) · bitsandbytes 0.50.1 |
>
> torch 빌드에 따라 결과 방향이 달라, 두 빌드를 모두 기록한다. **"CUDA 활성" 행이 실제 `_run_basic_check` 실행 순서(GPU 먼저 실행 → 같은 프로세스에서 CPU 기준선 측정)와 같은 조건이다.**
>
> | 경로 | 조건 | float32 | bfloat16 | bf16이 더 빠른가 |
> |---|---|---|---|---|
> | bnb 4bit | CUDA 활성(cu128, 실제 실행 순서와 동일) | 42.57ms | 52.58ms | **아니오 — 1.23배 느림** |
> | bnb 4bit | CPU 단독(cu 미설치) | 23.38ms | 20.89ms | 예 — 1.12배 빠름 |
> | `nn.Linear` 폴백 | CUDA 활성(cu128) | 11.87ms | 8.79ms | 예 — 1.35배 빠름 |
> | `nn.Linear` 폴백 | CPU 단독 | 11.24ms | 6.37ms | 예 — 1.77배 빠름 |
>
> `nn.Linear` 폴백 경로는 두 torch 빌드 모두에서 bf16이 float32보다 일관되게 빨랐다(1.35~1.77배) — 이 CPU가 `avx512_bf16` 전용 명령어 없이도 bf16 텐서의 메모리 대역폭 이점만으로 이득을 보는 것으로 보인다. 반면 **bnb 4bit 경로는 방향 자체가 뒤집혔다** — 실제 실행 순서와 같은 "CUDA 활성" 조건에서는 bf16이 오히려 23% 느렸다. GPU fp16 대비 배수로 보면 4bit 경로는 16.90배(float32) → 20.87배(bf16)로 오히려 배수가 더 벌어지고, linear 경로는 6.52배(float32) → 4.83배(bf16)로 좁혀져 — **같은 CPU 안에서도 backend에 따라 배수가 반대 방향으로 움직인다.**
>
> 결론: 이 두 번째 조합은 "bf16이 항상 더 공정한 기준선"이라는 가설을 뒷받침하지 않는다 — `nn.Linear` 경로에서는 지지하지만 실제 기본 체크가 우선 시도하는 `bnb 4bit` 경로(그리고 실제 실행 순서인 CUDA 활성 조건)에서는 반대다. 조합 1개 → 2개가 됐을 뿐이고 Issue #3이 원래 요구한 "여러" 조합에는 아직 못 미치므로, 이 결과만으로 `worker.py`의 dtype 선택 로직을 바꾸지는 않는다. 원본 데이터·재현 스크립트는 Issue #3 댓글 참고.
