...
5. 요구사항, 영향 파일, 구현 계획, 테스트 계획, 위험 요소를 한 번 보고한다.
   - **테스트 계획에는 실환경 재현 검증 적용 여부를 반드시 명시한다 — 적용/생략 중 하나와
     그 이유를 한 줄로 적는다.** 판단은 "pytest로 충분해 보인다"에서 시작하지 않는다.
     먼저 AGENTS.md "실환경 재현 검증 예외"의 **적용 대상** 목록에 이 Issue가 해당하는지부터
     확인한다:
     - [ ] QA에서 발견되어 이슈화된 버그 픽스인가
     - [ ] `@requires_cuda`/torch 관련 경로를 건드리는가
     - [ ] Windows 전용 동작을 건드리는가
     - [ ] 설치·`--yes` 자동수정·출력 가독성 관련 동작을 건드리는가
     하나라도 해당하면 원칙적으로 포함이다. "로직 자체는 순수해서 pytest로 이미
     커버된다"는 위 네 개에 **하나도 안 걸릴 때만** 쓰는 생략 사유지, 걸리는 항목을
     되짚어 빼는 사유로 쓰지 않는다(#84에서 이 순서를 거꾸로 적용해 실환경 검증을
     빠뜨린 적 있다).
     GPU가 필요한 항목인데 이 세션에 실제 GPU 유무 정보가 없으면, "GPU가 없어 못 본다"고
     단정하기 전에 먼저 사용자 메모리에 하드웨어 정보가 있는지 확인한다.
